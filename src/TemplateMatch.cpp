...
void  TemplateMatch::on_Mouse_LEFT(int event, int x, int y, int, void*)
  ...
  void TemplateMatch::Mouse_getColor(int event, int x, int y, int, void*)
  {
	  ...
	  case CV_EVENT_LBUTTONUP:
		  ...
		  {
			  uchar*data = image_l.ptr<uchar>(j);
  ...
  void TemplateMatch::auto_Template()
  {
	  for (int picture = 0; picture < 2; picture++)
	  {
		  ...
		  if (picture == 0)
		  {
			  gray_image = image_l.clone();
			  temporary = image_l.clone();
		  }
		  else
		  {
			  gray_image = image_r.clone();
			  temporary = image_r.clone();
		  }
		  ...
		  if (contours.size() == 6)
		  {
			  ...
			  while (it != contours.end())
			  {
				  ...
				  Template_batch[picture * 6 + i] = temporary(cv::Rect(start_point[picture][i] - cv::Point(size_template / 2, size_template / 2),
					  start_point[picture][i] + cv::Point(size_template / 2, size_template / 2)));
				  cv::circle(temporary, start_point[picture][i], 2, cv::Scalar(0, 255, 0), 2);
				  i++;
			  }
